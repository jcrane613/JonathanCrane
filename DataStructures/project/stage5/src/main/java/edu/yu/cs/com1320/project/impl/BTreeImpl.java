...
//max children per B-tree node = MAX-1 (must be an even number and greater than 2)
	...
	private DocumentPersistenceManager per;
	...
	private Entry get(Node currentNode, Key key, int height) throws IOException {
		...
		if (height == 0)
		{
			for (int j = 0; j < currentNode.entryCount; j++)
			{
				...
				{
					//found desired key
					if (entries[j].val == null) //if the val is null, that means it was deserialzed. So if the person wants it must deserilzie to get it
					{
						//need to account if there is nothign there to bedeserialzed. If so, should just reutn the vlaue, which in this case is null
						entries[j].val = per.deserialize((URI) key);
						return entries[j];
					}
		...
		else
		{
			for (int j = 0; j < currentNode.entryCount; j++)
			{
				...
				if (j + 1 == currentNode.entryCount || less(key, entries[j + 1].key))
				{
					...
					//this code will be using the above code to return a doc that was deserialzed
				}
			}
			//didn't find the key
			return null;
	...
	public Value put(Key key, Value val) {
		...
		Entry alreadyThere = null;
		try
		{
			alreadyThere = (Entry) this.get(this.root, key, this.height);
		} catch (IOException e)
		{
			e.printStackTrace();
		}
		...
		{
			Value returnValue = (Value) alreadyThere.val;
		...
		this.height++;
		return null; //double check that this is correct, since I added this
	}
	...
	public void moveToDisk(Key k) throws Exception {
		per.serialize((URI) k, (Document) get(k));
		put(k, null); // is null, that means it was deserialed. If nothing is there when it tries to deserialize, that menas this doc was actaully delted
	}

	@Override
	public void setPersistenceManager(PersistenceManager<Key, Value> pm) {
		this.per = (DocumentPersistenceManager) pm;
	}
