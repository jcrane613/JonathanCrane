...
import static edu.yu.cs.com1320.project.stage2.DocumentStore.DocumentFormat.*;
...
public class DocumentStoreImplTest {
	...
	@Test
	public void nullInputReturn() throws URISyntaxException {
		String text = "Hey, I hope you're having a nice day";
		InputStream input = new ByteArrayInputStream(text.getBytes());
		DocumentStoreImpl docStore = new DocumentStoreImpl();
		URI uri = new URI("https://piazza.com/class/k5fvaanpgc56kx");
		docStore.putDocument(input, uri, TXT);
		docStore.putDocument(new ByteArrayInputStream(this.pdfData1),this.uri1, PDF);
		URI uri2 = new URI("https://www.amazon.com/");
		InputStream nullInput = null;
		InputStream nullInput2 = null;
		assertEquals(0,docStore.putDocument(nullInput,uri2,TXT));//deleted a doc that doesn't exist
	...
	public void putDocumentExists() throws URISyntaxException{
		try {
			new DocumentStoreImpl().putDocument(null, new URI("hi"), PDF);
		} catch (Exception e) {}
	}
	...
	public void testPutPdfDocumentNoPreviousDocAtURI(){
		...
		int returned = store.putDocument(new ByteArrayInputStream(this.pdfData1),this.uri1, PDF);
	...
	public void testPutNewVersionOfDocumentPdf(){
		...
		returned = store.putDocument(new ByteArrayInputStream(this.pdfData2),this.uri1, PDF);
		//TODO allowing for student following old API comment. To be changed for stage 2 to insist on following new comment.
	...
	public void testDeleteDoc(){
		DocumentStore store = new DocumentStoreImpl();
		store.putDocument(new ByteArrayInputStream(this.pdfData1),this.uri1, PDF);
	...
	public void undoFunction()
	{
		DocumentStore store = new DocumentStoreImpl();
		store.putDocument(new ByteArrayInputStream(this.pdfData1),this.uri1, PDF);
		store.putDocument(new ByteArrayInputStream(this.pdfData2),this.uri2, PDF);
		store.undo();
		assertFalse(store.deleteDocument(this.uri2));
		store.undo(this.uri1);
		assertFalse(store.deleteDocument(this.uri1));
	}
	//New Testing for Stage2
	@Test
	public void undoEmptyStack() {
		try//undo without uri
		{
			DocumentStore store = new DocumentStoreImpl();
			store.undo();
		...
		try//undo and can't find this uri
		{
			DocumentStore store = new DocumentStoreImpl();
			store.putDocument(new ByteArrayInputStream(this.pdfData1),this.uri1, PDF);
			store.putDocument(new ByteArrayInputStream(this.pdfData2),this.uri2, PDF);
			URI tester = URI.create("https://me.com/class/k5fvaanpgc56kx");
			store.undo(tester);
		} catch (IllegalStateException e) { }
	}
	@Test
	public void workingUndo() throws URISyntaxException {
		DocumentStore store = new DocumentStoreImpl();
		store.putDocument(new ByteArrayInputStream(this.pdfData1), this.uri1, PDF);
		store.undo(this.uri1);
		store.putDocument(new ByteArrayInputStream(this.pdfData2), this.uri2, PDF);
		...
		store.putDocument(new ByteArrayInputStream(this.txt1.getBytes()), jonathan, TXT);
		store.undo();
		assertFalse(store.deleteDocument(jonathan));
		store.putDocument(new ByteArrayInputStream(this.txt2.getBytes()), moshe, TXT);
		store.undo(jonathan);
		assertTrue(store.deleteDocument(moshe));
	}
	@Test
	public void nullInputUndo()
	{
		DocumentStore store = new DocumentStoreImpl();
		store.putDocument(null, this.uri1, PDF);
		store.undo();
	}
	}
